...
> 取消别名：unalias或者在命令前加\
> 快捷键：^D(退出/logout) ^A(移到命令最前面) ^E(移到命令最后面) ^L() ^U(删除当前行所有命令) ^K(删除光标后面的所有) ^S(锁住命令行) ^Q(解除锁定命令行) ^Y(撤销删除) ^T 交换光标处文字次序
...
> -le 小于或等于               应用于：整形比较
> -ge 大于或等于               应用于：整形比较
> -a 双方都成立（and）用法：   逻辑表达式 -a 逻辑表达式
> -o 单方成立 （or）  用法：   逻辑表达式 -o 逻辑表达式
> -z 空字符串
# if使用
> if (($num1+/-/\*//$num2));then
> ...
...
> -amin  [+-]n 分钟
> -cmin  [+-]n 分钟
> -type [f(普通文件)/d(目录)/b(块设备文件)/c(字符设备文件)]
